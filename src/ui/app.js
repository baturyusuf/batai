...
function renderMeetingDetail(meeting) {
  ...
  const contextRecords = (meeting.contextPackages ?? []).slice(-8).reverse();
  const contextCards = contextRecords.map(record => `<article class="context-package"><header><strong>${esc(record.actorId)} · ${esc(label(record.status))}</strong><span>${number(record.estimatedTokens)} estimated input tokens</span></header><small class="mono">${esc(record.fingerprint?.slice(0,16) ?? '—')} · ${esc(record.baseSha?.slice(0,10) ?? 'uncommitted/unknown')}</small><ul>${(record.sources ?? []).map(source => `<li><b>${esc(label(source.sourceType))}</b> ${esc(source.sourceId)}<small>${esc(source.reasonIncluded)} · ${number(source.estimatedTokens)} tokens</small></li>`).join('') || '<li>No source content included</li>'}</ul>${record.excluded?.length ? `<details><summary>${record.excluded.length} excluded</summary><ul>${record.excluded.map(item => `<li>${esc(item.sourceId)} — ${esc(item.reason)}</li>`).join('')}</ul></details>` : ''}${record.warnings?.length ? `<p class="context-warning">${esc(record.warnings.join(' · '))}</p>` : ''}</article>`).join('');
  const gates = (meeting.executionGates ?? []).map(gate => `<div class="execution-gate"><strong>${esc(label(gate.status))}</strong><span>${esc(gate.provider ?? '—')} / ${esc(gate.model ?? '—')} · ${esc(gate.safeContinuationPhase)}</span><small>${esc(gate.resolutionReason ?? `Waiting for exact decision ${gate.decisionId ?? ''}`)}</small></div>`).join('');
  ...
  const detailSections = $$('.meeting-section', $('#meeting-detail'));
  const agendaSection = detailSections.find(section => section.querySelector('h3')?.textContent === 'Agenda');
  if (gates && agendaSection) agendaSection.insertAdjacentHTML('beforebegin', `<section class="meeting-section"><h3>Execution gate</h3>${gates}</section>`);
  const contributionSection = $$('.meeting-section', $('#meeting-detail')).find(section => section.querySelector('h3')?.textContent === 'Contributions');
  if (contributionSection) contributionSection.insertAdjacentHTML('beforebegin', `<section class="meeting-section"><h3>Context</h3><p class="metric-note">Estimated context/input usage is separate from actual provider usage.</p>${contextCards || '<p class="empty">Context will be built at the provider-safe boundary.</p>'}</section>`);
...
$('#meeting-form').addEventListener('submit', async event => {
  ...
  const request = {
    ...
    explicitFiles:form.elements.explicitFiles.value.split('\n').map(item=>item.trim()).filter(Boolean),
    contextBudget:{maxTokens:Number(form.elements.contextTokens.value),maxFiles:Number(form.elements.contextFiles.value),maxBytesPerFile:65536,maxTotalBytes:196608,maxHistoricalArtifacts:8}
  };
