...
Economic PAYG permission and live provider-operation approval are separate execution gates. A PAYG decision authorizes only its bound operation/resource/model/budget/context/routing scope. Context packages are checked after resource acquisition and again immediately before `send_task`; a bounded rebuild/reroute is attempted before any provider call. Estimated input size is recorded separately from actual provider-reported input tokens.
