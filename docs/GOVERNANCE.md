...
Governance, live provider approvals and GitHub delivery are shared by the Rust desktop, HTTP and MCP control surfaces. MCP actors are Director, HTTP human mutations require a local control token, and neither transport can bypass `AuthorityRouter`. Deprecated Node entry files are test references only. Meeting scheduling/execution and automatic summaries are typed foundations, not a completed meeting product. A provider request cannot survive the death of its original App Server process; restart therefore orphans it and deliberately requires review rather than replay.
