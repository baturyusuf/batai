...
The allowlisted command surface maps to the same `ControlService` and Rust domain services used by the daemon. It includes snapshots, resources, governed organization/task operations, bounded meetings, delivery, provider approvals and the legacy Director MCP operations. Transports do not write files, run providers or open SQLite directly. Meeting turns continue when Desktop or MCP disconnects because only the daemon owns them. A meeting waiting on an execution gate also counts as active runtime work.

GOD decision resolution is a daemon event. The Meeting Engine validates the exact resource/model/policy/configuration binding, refreshes bounded context and resumes only from a persisted pre-turn boundary. Startup reconciliation handles decisions resolved while every client was disconnected.
